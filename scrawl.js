...
var scrawl = function () {
    ...
    var o = {
        ...
        _addBoardListener:function (saveNum) {
            ...
            margin = parseInt($("#J_wrap").attr("margin-left"));
            drawStep.push(context.getImageData(0, 0, context.canvas.width, context.canvas.height));
            drawStepIndex += 1;

            canvas.on("mousedown, mousemove, mouseup, mouseout", function(e) {
        ...
        _addOPerateListener:function (saveNum) {
            var me = this;
           $("#J_previousStep").on("click", function () {
                if (drawStepIndex > 1) {
                    drawStepIndex -= 1;
            ...
            $("#J_clearBoard").on("click", function () {
                context.clearRect(0, 0, context.canvas.width, context.canvas.height);
                drawStep = [];
                me._saveOPerate(saveNum);
                drawStepIndex = 1;
        ...
        _addColorBarListener:function () {
            var me = this;
            $("#J_colorBar").on("click", function (e) {
        ...
        _addBrushBarListener:function () {
            var me = this;
            $("#J_brushBar").on("click", function (e) {
                var target = me.getTarget(e),
        ...
        _addEraserBarListener:function () {
            var me = this;
            $("#J_eraserBar").on("click", function (e) {
                var target = me.getTarget(e),
                    size = $.browser.ie ? target.innerText : target.text;
                if (!!size) {
                    me._addBESelect(target);
        ...
        _addAddImgListener:function () {
            ...
            if (!window.FileReader) {
               $("#J_addImg").style.display = 'none';
               $("#J_removeImg").style.display = 'none';
               $("#J_sacleBoard").style.display = 'none';
            }
        ...
        _originalColorSelect:function (title) {
            var colorList =$("#J_colorList").find("td");
        ...
        _originalBrushSelect:function (text) {
            var brushList =$("#J_brushBar").children;
        ...
        _addColorSelect:function (target) {
            var me = this,
                colorList =$("#J_colorList").getElementsByTagName("td"),
                eraserList =$("#J_eraserBar").children,
                brushList =$("#J_brushBar").children;
        ...
        _addBESelect:function (target) {
            var brushList =$("#J_brushBar").children;
            var eraserList =$("#J_eraserBar").children;
    ...
    ScaleBoy.prototype = {
        init:function () {
            ...
            $(scale).on('mousedown', function (e) {
                ...
                $(document.body).on('mousemove', me.scaleMousemove);
